 ...
 */
GitHubEnterpriseClient = function(editorUi)
{
	DrawioClient.call(this, editorUi, 'ghauth');
};

// Extends DrawioClient
mxUtils.extend(GitHubEnterpriseClient, DrawioClient);
 ...
 */
GitHubEnterpriseClient.prototype.clientId = 'cb0fe57275d25b8e05f6';

/**
 * OAuth scope.
 */
GitHubEnterpriseClient.prototype.scope = 'repo';

/**
 * Default extension for new files.
 */
GitHubEnterpriseClient.prototype.extension = '.xml';
 ...
 */
GitHubEnterpriseClient.prototype.maxFileSize = 1000000 /*1MB*/;

/**
 * Authorizes the client, gets the userId and calls <open>.
 */
GitHubEnterpriseClient.prototype.updateUser = function(success, error, failOnAuth)
{
 ...
 * Authorizes the client, gets the userId and calls <open>.
 */
GitHubEnterpriseClient.prototype.authenticate = function(success, error)
{
	if (window.onGitHubEnterpriseCallback == null)
	{
		var auth = mxUtils.bind(this, function()
		{
			...
			{
				var win = window.open('https://git.realestate.com.au/login/oauth/authorize?client_id=' +
					this.clientId + '&scope=' + this.scope, 'ghauth');
				
				if (win != null)
				{
					window.onGitHubEnterpriseCallback = mxUtils.bind(this, function(code, authWindow)
					{
						if (acceptAuthResponse)
						{
							window.onGitHubEnterpriseCallback = null;
 ...
 * Authorizes the client, gets the userId and calls <open>.
 */
GitHubEnterpriseClient.prototype.executeRequest = function(req, success, error)
{
 ...
 */
GitHubEnterpriseClient.prototype.getLibrary = function(path, success, error)
{
 ...
 * Checks if the client is authorized and calls the next step.
 */
GitHubEnterpriseClient.prototype.getFile = function(path, success, error, asLibrary, checkExists)
{
		...
		(!this.ui.useCanvasForExport && binary)))
	{
		...
		if (this.token != null)
		{
			var url = this.baseUrl + '/repos/' + org + '/' + repo + '/contents/' +
				path + '?ref=' + ref + '&access_token=' + this.token;
	...
	{
		var uri = this.baseUrl + '/repos/' + org + '/' + repo + '/contents/' + path + '?ref=' + ref  + '&access_token=' + this.token;
		console.log(uri);
		var req = new mxXmlRequest(uri, null, 'GET');
		
		this.executeRequest(req, mxUtils.bind(this, function(req)
		{
			try
			{
				success(this.createGitHubEnterpriseFile(org, repo, ref, JSON.parse(req.getText()), asLibrary));
			}
			catch (e)
			{
				error(e);
 ...
 */
GitHubEnterpriseClient.prototype.createGitHubEnterpriseFile = function(org, repo, ref, data, asLibrary)
{
	...
	return (asLibrary) ? new GitHubEnterpriseLibrary(this.ui, content, meta) : new GitHubEnterpriseFile(this.ui, content, meta);
};
 ...
 * @param {number} dy Y-coordinate of the translation.
 */
GitHubEnterpriseClient.prototype.insertLibrary = function(filename, data, success, error, folderId)
{
 ...
 * @param {number} dy Y-coordinate of the translation.
 */
GitHubEnterpriseClient.prototype.insertFile = function(filename, data, success, error, asLibrary, folderId, base64Encoded)
{
	asLibrary = (asLibrary != null) ? asLibrary : false;
	...
	this.checkExists(org + '/' + repo + '/' + ref + '/' + path, true, mxUtils.bind(this, function(checked, sha)
	{
		if (checked)
		{
			...
			if (!asLibrary)
			{
				success(new GitHubEnterpriseFile(this.ui, data, {'org': org, 'repo': repo, 'ref': ref,
				...
				this.showCommitDialog(filename, true, mxUtils.bind(this, function(message)
				{
					this.writeFile(org, repo, ref, path, message, data, sha, mxUtils.bind(this, function(req)
					{
						try
						{
							...
							success(this.createGitHubEnterpriseFile(org, repo, ref, msg.content, asLibrary));
						}
						catch (e)
						{
							error(e);
 ...
 * 
 */
GitHubEnterpriseClient.prototype.showCommitDialog = function(filename, isNew, success, cancel)
{
 ...
 * 
 */
GitHubEnterpriseClient.prototype.writeFile = function(org, repo, ref, path, message, data, sha, success, error)
{
	...
	else
	{
		...
		var uri = this.baseUrl + '/repos/' + org + '/' + repo + '/contents/' + path + '?access_token=' + this.token;
		console.log(uri);
		var req = new mxXmlRequest(uri, JSON.stringify(entity), 'PUT');
 ...
 * @param {number} dy Y-coordinate of the translation.
 */
GitHubEnterpriseClient.prototype.checkExists = function(path, askReplace, fn)
{
 ...
 * @param {number} dy Y-coordinate of the translation.
 */
GitHubEnterpriseClient.prototype.saveFile = function(file, success, error)
{
 ...
 * Checks if the client is authorized and calls the next step.
 */
GitHubEnterpriseClient.prototype.pickLibrary = function(fn)
{
	this.pickFile(fn);
};
 ...
 * Checks if the client is authorized and calls the next step.
 */
GitHubEnterpriseClient.prototype.pickFolder = function(fn)
{
	this.showGitHubEnterpriseDialog(false, fn);
};

/**
 * Checks if the client is authorized and calls the next step.
 */
GitHubEnterpriseClient.prototype.pickFile = function(fn)
{
	fn = (fn != null) ? fn : mxUtils.bind(this, function(path)
	{
		this.ui.loadFile('E' + encodeURIComponent(path));
	});
	
	this.showGitHubEnterpriseDialog(true, fn);
};

/**
 * 
 */
GitHubEnterpriseClient.prototype.showGitHubEnterpriseDialog = function(showFiles, fn)
{
	var org = null;
	var repo = null;
	var ref = null;
	...
	var createLink = mxUtils.bind(this, function(label, fn)
	{
		...
		mxUtils.write(link,  label); // can we add some icon here to indicate branch v dir v file
	...
	{
		var uri = this.baseUrl + '/repos/' + org + '/' + repo + '/contents/' + path + '?ref=' + encodeURIComponent(ref)  + '&access_token=' + this.token;
	...
	// Adds paging for repos and branches (files limited to 100 by API)
	var pageSize = 100;
	...
	var selectRef = mxUtils.bind(this, function(page)
	{
		...
		var uri = this.baseUrl + '/repos/' + org + '/' + repo + '/branches?per_page=' + pageSize + '&page=' + page  + '&token=' + this.token;
		console.log(uri);
		var req = new mxXmlRequest(uri, null, 'GET');
	...
	var selectRepo = mxUtils.bind(this, function(page)
	{
		...
		var uri = this.baseUrl + '/user/repos?per_page=' + pageSize + '&page=' + page + '&token=' + this.token;
		console.log(uri);
		var req = new mxXmlRequest(uri, null, 'GET');
		dlg.okButton.setAttribute('disabled', 'disabled');
		this.ui.spinner.spin(div, mxResources.get('loading'));
		...
		this.executeRequest(req, mxUtils.bind(this, function(req)
		{
			...
			else
			{
				if (page == 1)
				{
					...
					{
						var dlg = new FilenameDialog(this.ui, '<org>/<repository>/<branch>', mxResources.get('ok'), mxUtils.bind(this, function(value)
						{
							if (value != null)
							{
 ...
 * Checks if the client is authorized and calls the next step.
 */
GitHubEnterpriseClient.prototype.logout = function()
{
